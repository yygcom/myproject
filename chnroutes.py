...
def generate_ss(metric):
    results = fetch_ip_data()
    upscript_header=textwrap.dedent("""\
#!/bin/bash
export PATH="/bin:/sbin:/usr/sbin:/usr/bin"
iptables -t nat -N SHADOWSOCKS
iptables -t nat -A SHADOWSOCKS -d 172.20.3.0/24 -p tcp -j RETURN
...
iptables -t nat -A SHADOWSOCKS -d 240.0.0.0/4 -j RETURN
    ...
    upfile.write(upscript_header)
    upfile.write('\n')
    upfile.write(upscript_liwai)
    
    for ip,mask,mask2 in results:
        #print mask2
        upfile.write('iptables -t nat -A SHADOWSOCKS -d %s/%s -p tcp -j RETURN\n'%(ip,mask2))

    upfile.write(upscript_footer)
    print "For SS iptables rules created ok"
...
if __name__=='__main__':
    ...
    elif args.platform.lower() == 'ss':
        generate_ss(args.metric)
